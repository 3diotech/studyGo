  ...
  render() {
    ...
    return (
      <View style={{ height: Dimensions.get('screen').height, width: '100%', backgroundColor: "#4CCEEB", display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <View style={{ backgroundColor: 'white', width: '80%', height: 350, borderRadius: 5, marginTop: 60, padding: 25 }}>
          ...
          <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-end', marginTop: 15 }}>
            <View style={styles.tag}>
              <Text>Multiple Choice</Text>
            </View>

            <View style={styles.tag}>
              <Text>{currentQuestion.tag}</Text>
            </View>
          </View>
        </View>

        {!this.state.tackle ? <View style={{ marginTop: 50 }}>
